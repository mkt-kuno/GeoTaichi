...
Threshold = 1e-12
@ti.data_oriented
class StaggeredQuadrilateralElement(ElementBase):
    ...
    def create_nodes(self, sims: Simulation, grid_size):
        ...
        for d in range(2):
            if gnum[d] == 0:
                gnum[d] = 1
        gnum = np.array(8 * np.ceil(gnum / 8), dtype=np.int32)
        
        self.grid_size = vec2f(domain / gnum)
        self.igrid_size = 1. / self.grid_size
        self.cell_volume = self.calc_volume()
        self.gnum = vec2i(gnum)
        ...
        for np in range(particleNum):
            ...
            for j in range(base_bound[1], base_bound[1] + influenced_node):
                ...
                for i in range(base_bound[0], base_bound[0] + influenced_node):
                    ...
                    shapen0, shapen1 = self.shapefn(particle[np].x, node_coords, ielement_size, psize, shape_function)
                    shapeval = shapen0 * shapen1
                    if shapeval > Threshold:
                        dshapen0, dshapen1 = self.grad_shapefn(particle[np].x, node_coords, ielement_size, psize, grad_shape_function)
    ...
    def cal_min_grid_size(self):
        return ti.min(self.grid_size[0], self.grid_size[1])