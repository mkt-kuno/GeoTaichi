...
from . import mesh 
from ..sdf.LevelSetGrid import LocalGrid
from ..sdf.BuildSurfaceNode import RayTracing
from ..sdf.FastMarchingMethod import FastMarchingMethod
from ..utils.linalg import (heaviside_function, transformation_matrix_coordinate_system, generate_grid)
from ..utils.Root import newton
import trimesh as tm
try:
    from trimesh.interfaces import gmsh
except ImportError:
    gmsh = None  # gmsh not available in this version of trimesh
...
class BasicShape(object):
    ...
    def tetrahedization(self, max_length=None, algorithm=1, refine_number=0):
        if self.tetramesh is None:
            if gmsh is None:
                raise ImportError("gmsh module is not available in this version of trimesh. Please upgrade trimesh or use an alternative method.")
