...
from ..dem.engines.ExplicitEngine import ExplicitEngine as DEMExplicitEngine
from ..dem.engines.EngineKernel import get_contact_stiffness, get_gain, servo, get_wall_contact_force
...
from ..mpm.engines.ULExplicitEngine import ULExplicitEngine as MPMExplicitEngine
...
class Engine(object):
    ...
    def manage_function(self, drag_model):
        ...
        if self.sims.coupling_scheme == 'MPDEM' or self.sims.coupling_scheme == 'DEMPM':
            ...
            from ..mpdem.fluid_dynamics.CFDpart import CFDMPM
            if self.dsims.scheme == "DEM":
                self.compute = self.CFDEMintegration
            elif self.dsims.scheme == "LSDEM":
                self.compute = self.CFDEMlsintegration
